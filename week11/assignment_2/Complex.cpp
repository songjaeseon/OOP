...
Number *Complex::add(Number * num) {
     ...
     */
    if (num->types() == INTEGER) { //map을 활용하여 복소수타입의 결과값을 반환한다.
        Integer* result = dynamic_cast<Integer*>(num);
	Number* dv = new Complex(this->value.first + result->val(), this->value.second);
        delete(num);
        return dv;

    } else if (num->types() == FLOAT) {
        Float* result = dynamic_cast<Float*>(num);
        Number* dv = new Complex(this->value.first + result->val(), this->value.second);
        delete(this);
        return dv;

    } else if (num->types() == COMPLEX) {
        Complex* result = dynamic_cast<Complex*>(num);
        Number* dv = new Complex(this->value.first + result->real(), this->value.second + result->imag());
        delete(this);
        return dv;
    } 
    return nullptr;
...
Number *Complex::mul(Number * num) {
    ...
    if (num->types() == INTEGER) {
        Integer* result = dynamic_cast<Integer*>(num);
        Number* dv = new Complex(this->value.first * result->val(), this->value.second * result->val() );
        delete(num);
        return result;
        ...
        Complex* result = dynamic_cast<Complex*>(num);
        Number* dv = new Complex((this->real() * result->real()) -(this->imag() * this->imag()),( this->real() * result->imag())+( this->imag()*result->real()));
...
Number *Complex::div(Number * num) {
    ...
    if (num->types() == INTEGER) {
        Integer* result = dynamic_cast<Integer*>(num);
        Number* a  = new Complex(this->real() / result->val(), this->imag() / result->val());
        return a;
