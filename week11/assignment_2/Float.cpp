...
Number *Float::add(Number * num) {
     ...
     */
    if (num->types() == INTEGER) { //num타입이 this보다 크기 때문에 객체를 새로 생성한다
        Integer* result = dynamic_cast<Integer*>(num);
	delete(num);
	Number* b = new Float(result->val() + this->val()); //Number객체안에 float타입 결과값을 저장한다
        return b;

    } else if (num->types() == FLOAT) {
        Float* result = dynamic_cast<Float*>(num);
        result->set_val(this->val() + result->val());
        delete(this);
        return result;

    } else if (num->types() == COMPLEX) {
        Complex* result = dynamic_cast<Complex*>(num);
        result->set_val(result->real()+ this->val(), result->imag());
        delete(this);
        return result;
...
Number *Float::sub(Number * num) {
     ...
     */
     if (num->types() == INTEGER) {
        Integer* result = dynamic_cast<Integer*>(num);
        delete(num);
        Number* b = new Float(this->val() - result->val());
        return b;
    } else if (num->types() == FLOAT) {
        Float* result = dynamic_cast<Float*>(num);
       	result->set_val(this->val() - result->val());
        ...
        Complex* result = dynamic_cast<Complex*>(num);
        result->set_val(this->val() - result->real(), result->imag());
        delete(this);
        return result;
...
Number *Float::mul(Number *num) {
    ...
    if (num->types() == INTEGER) {
        Integer* result = dynamic_cast<Integer*>(num);
        delete(num);
        Number* b = new Float(result->val() * this->val());
        return b;
        ...
        Complex* result = dynamic_cast<Complex*>(num);
        result->set_val(this->val() * result->real(), (result->imag()*this->val()));
        delete(this);
        return result;
